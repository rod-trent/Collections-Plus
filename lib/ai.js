...
// Hosts that expose sensitive cloud/internal metadata and must never be
// reachable via a user-supplied provider base URL (SSRF hardening).
const BLOCKED_AI_HOSTS = new Set(['169.254.169.254', 'metadata.google.internal', '[::1]']);

/** Reject provider URLs that aren't plain http(s) or that target metadata hosts. */
function assertSafeProviderUrl(url) {
  let parsed;
  try {
    parsed = new URL(url);
  } catch {
  ...
  if (BLOCKED_AI_HOSTS.has(parsed.hostname)) {
    throw new Error('AI provider URL points to a disallowed host.');
  }
}
