...
import { checkAiBaseUrl } from './netguard.js';
...
export async function chat({ config, system, messages, signal }) {
  ...
  const badUrl = checkAiBaseUrl(resolved.baseUrl);
  ...
  try {
    res = await fetch(url, {
      ...
      signal,
      credentials: 'omit',
      // A redirect would re-send the API key (and the collection data) to
      // wherever the provider points us; refuse rather than follow.
      redirect: 'error',
    });
  } catch (err) {
