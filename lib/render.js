...
export function collectionToMarkdown(c) {
  ...
  for (const it of c.items || []) {
    ...
    if (it.type === 'note') {
      ...
      const quote = mdEscape((it.text || '').replace(/\s*\n\s*/g, ' '));
      line = it.url ? `- ${box} “${quote}” — [source](${it.url})` : `- ${box} “${quote}”`;
...
function itemToHtml(it) {
  ...
  if (it.type === 'highlight') {
    const quote = `<span class="note">“${htmlEscape(it.text || '')}”</span>`;
    const src = it.url ? ` <a href="${htmlEscape(it.url)}">(source)</a>` : '';
    const note = it.note ? ` — ${htmlEscape(it.note)}` : '';
    return `${box}${quote}${src}${note}${extraHtml}`;
  }
  if (it.type === 'image') {
    const url = it.srcPageUrl || it.src || '';
...
function itemToCard(it) {
  ...
  if (it.type === 'highlight') {
    const src = it.url
      ? `\n        <a class="card-host" href="${htmlEscape(it.url)}">${htmlEscape(it.title || 'source')}</a>`
      : '';
    const note = it.note ? `\n        <p class="card-note-text">${htmlEscape(it.note)}</p>` : '';
    return `    <li class="card card-note">
      <blockquote class="card-quote">${htmlEscape(it.text || '')}</blockquote>${src}${note}
    </li>`;
  }
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .card-quote { margin: 14px; padding-left: 12px; border-left: 3px solid var(--accent);
