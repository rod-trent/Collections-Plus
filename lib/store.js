...
function migrate(data) {
  ...
  // Drop any collection parentId that doesn't point at an existing folder or collection.
  const folderIds = new Set(data.folders.map((f) => f.id));
  for (const c of data.collections) {
    if (c.parentId && !folderIds.has(c.parentId) && !findCollection(data, c.parentId) && !data.archive.find((col) => col.id === c.parentId) && !data.trash.find((e) => e.collection.id === c.parentId)) c.parentId = null;
  }
  assignOrder(data);
  ...
  data.archive = (Array.isArray(data.archive) ? data.archive : []).map((c) => {
    ...
    mc.parentId = findCollection(data, c.parentId) || data.archive.find((col) => col.id === c.parentId) || data.trash.find((e) => e.collection.id === c.parentId) ? c.parentId : null;
...
function assignOrder(data) {
  ...
  if (!anyOrder) {
    ...
    for (const f of data.folders) {
      let k = 0;
      for (const c of data.collections) if (c.parentId === f.id) c.order = k++;
    }
    return;
  }
  ...
  for (const f of data.folders) {
    const kids = data.collections.filter((c) => c.parentId === f.id);
...
export function findCollection(data, id) {
...
export async function createSubCollection(parentId, title = 'New subcollection') {
  let created;
  let item;
  await mutate((data) => {
    created = newCollection({ title,  parentId});
    //created = newCollection({ title});
    parent = data.collections.find( c => c.id === parentId );
    item = makeItem(
        {
          type: 'collection',
          cid: created.id
...
export async function findAllChildren(id) {
  const data = await getData();
  ...
  if (c) {
    for (const i of c.items) {
      if (i.type === 'collection') {
        children.add(i.cid);
        for (const child of await findAllChildren(i.cid)) {
          children.add(child);
        }
      }
    }
...
export async function setParent(id, parentId) {
  await mutate((data) => {
    ...
    if (c.parentId !== next) {
      ...
      if (next) {
        const kids = data.collections.filter((x) => x.parentId === next && x.id !== id);
        const max = Math.max(-1, ...kids.map((x) => x.order).filter((n) => Number.isFinite(n)));
        c.order = max + 1;
      } else {
        c.order = topOrderMax(data, id) + 1;
      }
    }
    c.updatedAt = now();
  });
...
export async function archiveCollection(id) {
  ...
  // Recursively archive any subcollections first
  const data = await getData();
  const c = findCollection(data, id);
  if (!c) return;
  ...
  for (const item of c.items) {
    if (item.type === 'collection') {
      await archiveCollection(item.cid);
    }
  }
...
export async function unarchiveCollection(id) {
  const data = await mutate((data) => {
    ...
    col.parentId = findCollection(data, col.parentId) ? col.parentId : null;
    col.updatedAt = now();
    data.collections.unshift(col);
    data.activeCollectionId = col.id;
  });

  // If collection has a parent, reattach it if necessary
  const col = findCollection(data, data.activeCollectionId);

  if (col.parentId) {
    const parent = findCollection(data, col.parentId);
    if (parent && !parent.items.find((i) => i.type === 'collection' && i.cid === col.id)) {
      await addItem(col.parentId, {type: 'collection', cid: col.id})
    }
  }

  // Recursively restore any subcollections
  for (const item of col.items) {
    if (item.type === 'collection') {
      const entry = data.archive.find((e) => e.id === item.cid);
      await unarchiveCollection(entry.id)
    }
  }
...
export async function trashCollection(id) {
  ...
  // Recursively trash any subcollections first
  const data = await getData();
  const c = findCollection(data, id);
  if (!c) return;
  ...
  for (const item of c.items) {
    if (item.type === 'collection') {
      await trashCollection(item.cid);
    }
  }
  await mutate((data) => {
    const i = data.collections.findIndex((c) => c.id === id);
...
export async function restoreFromTrash(entryId) {
  ...
  // Recursively restore any subcollections
  const restored = findCollection(data, data.activeCollectionId);
  for (const item of restored.items) {
    if (item.type === 'collection') {
      const entry = data.trash.find((e) => e.collection.id === item.cid);
      await restoreFromTrash(entry.id)
    }
  }
  // If collection has a parent, reattach it if necessary
  if (restored.parentId) {
    const parent = findCollection(data, restored.parentId);
    if (parent && !parent.items.find((i) => i.type === 'collection' && i.cid === restored.id)) {
      await addItem(parent.id, {type: 'collection', cid: restored.id});
    }
  }
}
...
export async function deleteTrashEntry(entryId) {
  // Recursively delete any subcollections first
  const data = await getData();
  const entry = data.trash.find((e) => e.id === entryId);
  if (entry.kind === 'collection') {
    for (const item of entry.collection.items) {
      if (item.type === 'collection') {
        const itemEntry = data.trash.find((e) => e.collection.id === item.cid);
        await deleteTrashEntry(itemEntry.id);
      }
    }
  }
...
export async function removeItem(collectionId, itemId) {
  ...
  await mutate((data) => {
    ...
    if (c) {
      ...
      if (item.type === 'collection') {
        childId = item.cid
        //const child = findCollection(data, childId);
        //child.parentId = null;
      }
...
export async function moveItems(fromId, itemIds, toId) {
  ...
  await mutate((data) => {
    ...
    for (const item of moving) {
      if (item.type === 'collection') {
        const c = findCollection(data, item.cid);
        c.parentId = toId
      }
    }
  });
...
export async function copyItems(fromId, itemIds, toId) {
  ...
  for (const col of cols) {
    const fromChild = findCollection(data, col);
    const items = fromChild.items.map(item => item.id);
    const toChild = await createSubCollection(toId, fromChild.title);
    await copyItems(fromChild.id, items, toChild.id);

    // copy cover by value
    if (fromChild.cover) {
      const newCover = await srcToCover(fromChild.cover);
      await setCover(toChild.id, newCover);
    }
  }
}
...
export async function importJSON(json, mode = 'merge') {
  ...
  await mutate((data) => {
    ...
    // Flatten folder/subcollection structure on merge so parentId can't dangle to old ids.
    for (const c of incoming.collections) {
      data.collections.push({
        ...c,
        id: uid(),
        parentId: null,
        items: (c.items || []).map((it) => ({ ...it, id: uid() })).filter((it) => it.type !== 'collection'),
      });
    }
  });
  return getData();
