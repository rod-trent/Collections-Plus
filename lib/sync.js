...
function normalize(parsed) {
  ...
  return { activeCollectionId, collections, folders };
}
...
async function writeFile(handle, stamp) {
  ...
  const payload = JSON.stringify(
    { version: data.version, activeCollectionId: data.activeCollectionId, collections: data.collections, folders: data.folders || [], exportedAt: stamp, app: 'Collections Plus' },
    null,
    2
  );
